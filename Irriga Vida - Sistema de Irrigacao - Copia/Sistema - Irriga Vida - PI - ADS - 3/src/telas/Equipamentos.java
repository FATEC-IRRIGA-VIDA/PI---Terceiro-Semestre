...
public class Equipamentos extends JFrame {
	...
	/***
	 * Metódo que executa uma pesquisa em toda a tabela de Equipamentos. (FILTRO)
	 */
	public void refreshTable() {
		...
		if(bd.getConnection()){ 
			...
			String sql ="select * from TB_EQUIPAMENTO"; // instrução executada no banco de dados.
			try {
	...
	public Equipamentos() {
		...
		btNovoCadastro.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Definindo condicional para a Data da Última Manutenção.
				//DateFormat df = new  SimpleDateFormat("yyyy-MM-dd");
				DateFormat df = new  SimpleDateFormat("dd-MM-yyyy");
				
				String dataUm = "0";
				if(dateChooserDataum.getDate() != null) {
					dataUm = (df.format(dateChooserDataum.getDate()));  //Formatação da DT_ULTIMA_MAN
				}
				else
