...
public class PlantiosUsuario2 extends JFrame {
	...
	/***
	 * Metódo que executa uma pesquisa em toda a tabela de Plantios. (FILTRO)
	 */
	public void refreshTable() {
		...
		if(bd.getConnection()){ 
			...
			String sql ="select * from TB_PLANTIO;"; // instrução executada no banco de dados.
			try {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					PlantiosUsuario2 frame = new PlantiosUsuario2();
	...
	public PlantiosUsuario2() {
		...
		btVoltar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				CadastrosUsuario.tela15.setVisible(false);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(bd.getConnection()){ 
					...
					String sql ="select * from TB_PLANTIO where ID_PLANTIO = ? or ID_PLANTA = ? or ID_FERTILIZANTE = ? or TIPO_ADUBO = ? "
							+ "or TEMPO_MED_IRRIGA = ? or METRAGEM = ? or QTD_SEMENTE = ?"; // instrução executada no banco de dados.
		...
		btAtualizar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(dateChooserInicioPlantio.getDate() != null && dateChooserFimPlantio.getDate() !=null
					...
					dtInicioP = (df.format(dateChooserInicioPlantio.getDate())); //Formatação da DT_INI_PLANTIO
					dtFimP = (df.format(dateChooserFimPlantio.getDate()));  //Formatação da DT_FIM_PLANTIO
					
					dtInicioC = (df.format(dateChooserInicioColheita.getDate())); //Formatação da DT_INI_COLHEITA
					dtFimC = (df.format(dateChooserFimColheita.getDate()));  //Formatação da DT_FIM_COLHEITA
				}
				else
					JOptionPane.showMessageDialog(null, "Insira datas válidas nos Campos");
				
				
				//Definindo a variavel a partir de cada combinação de seleção.
				String tipoPlantio; // -- H - Hortaliças | F - Frutas | L - Legumes
				
				if(ComboBoxTipoPlantio.getSelectedItem().equals("Hortaliças"))
				{
					tipoPlantio = "H";
				}
