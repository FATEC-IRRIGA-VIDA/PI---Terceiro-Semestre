...
public class Tarefas extends JFrame {
	...
	/***
	 * Metódo que executa uma pesquisa em toda a tabela de Tarefas. (FILTRO)
	 */
	public void refreshTable() {
		...
		if(bd.getConnection()){ 
			...
			String sql ="select * from TB_TAREFAS;"; // instrução executada no banco de dados.
			try {
		...
		btPesquisar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(bd.getConnection()){ 
					...
					String sql ="select * from TB_TAREFAS where ID_TAREFA = ? or ID_USUARIO = ? or DESCRICAO = ? or OBS_TAREFA = ? "; // instrução executada no banco de dados.
